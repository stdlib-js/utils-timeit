...
function timeit( code, options, clbk ) {
	...
	if ( !isString( code ) ) {
		throw new TypeError( format( '1Xn3F,Ex', code ) );
	}
	opts = copy( defaults );
	...
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '1Xn2b,GJ', cb ) );
	}
	...
	try {
		...
		err = new Error( format( '1XnBq,OJ', error.message ) );
		return done( err );
	}
	...
	*/
	function onTest( error, time ) {
		...
		if ( !isArray( time ) || time.length !== 2 ) {
			...
			error = new Error( format('1Xn1q,EY') );
			return done( error );
		}
